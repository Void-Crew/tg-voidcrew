...
/proc/find_clone_record(field, value, list/inserted_list)
...
/obj/machinery/computer/cloning/proc/Clone(mob/user, target)
	var/datum/data/record/C = find_clone_record("id", target, records)
...
/obj/machinery/computer/cloning/proc/scan_occupant(occupant, mob/user, body_only)
	...
	var/datum/data/record/old_record = find_clone_record("id", R.fields["id"], records)
	if(old_record)
		records -= old_record
